  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink">GPU Health &amp; Utilization</h1>
          <p className="text-sm text-ink-muted/60 mt-0.5">{gpu?.model_name ?? '—'}</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <MetricCard label="Temperature" value={gpu?.temperature_celsius?.toFixed(1) ?? null} unit="°C"
          accent={(gpu?.temperature_celsius ?? 0) >= 80 ? 'text-red-400' : 'text-ink'} />
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-surface-card border border-surface-border rounded-xl p-5 space-y-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold font-mono text-ink">{gpu?.sm_clock_mhz ?? '—'}<span className="text-sm font-normal text-ink-muted ml-1">MHz</span></p>
          </div>
          <div>
            <p className="text-xs text-ink-muted/60">MEM Clock</p>
            <p className="text-xl font-bold font-mono text-ink">{gpu?.mem_clock_mhz ?? '—'}<span className="text-sm font-normal text-ink-muted ml-1">MHz</span></p>
          </div>
          <div>
            <p className="text-xs text-ink-muted/60">GPU UUID</p>
            <p className="text-xs font-mono text-ink-muted truncate mt-0.5">{gpu?.gpu_uuid ?? '—'}</p>
          </div>
        </div>
      </div>

      <div className="bg-surface-card border border-surface-border rounded-xl p-5">
        <p className="text-xs text-ink-muted/60 uppercase tracking-wider mb-4">GPU Health Status</p>
        <div className="grid grid-cols-3 gap-4">
          <div className={`rounded-lg px-4 py-3 border ${hasXid ? 'bg-red-900/30 border-red-500/40' : 'bg-surface-card/50 border-surface-border/50'}`}>
          ...
          <div className={`rounded-lg px-4 py-3 border ${hasDbe ? 'bg-red-900/30 border-red-500/40' : hasSbe ? 'bg-amber-900/30 border-amber-500/40' : 'bg-surface-card/50 border-surface-border/50'}`}>
          ...
          <div className="rounded-lg px-4 py-3 border bg-surface-card/50 border-surface-border/50">
            ...
            <p className="text-2xl font-bold font-mono mt-1 text-ink">{gpu?.pcie_replay ?? '—'}</p>
          </div>
        </div>
      </div>
